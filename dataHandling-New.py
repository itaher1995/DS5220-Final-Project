...
def subsetCaptions(filePrefix,tokenMap):
    ...
    captionIndices = [i for i in range(len(captionDataFrame)) if intersectionEquals(captionDataFrame['caption'].iloc[i],tokenMap.values()) and len(captionDataFrame['caption'].iloc[i])<=config.MAX_CAP_LEN]
